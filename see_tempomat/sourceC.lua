...
addEventHandler("onClientElementDataChange", getPedOccupiedVehicle(localPlayer), 
	function (dataName, oldValue, newValue)
		local playerVehicle = getPedOccupiedVehicle(localPlayer)

		if dataName == "vehicle.engine" and newValue == 0 or newValue == false then 
			endTempomat()
		end
	end
)
...
function endTempomat()
	...
	exports.see_controls:toggleControl({"accelerate", "brake_reverse"}, true)
...
addCommandHandler("tempomat",
	function ()
		...
		if vehicle and getPedOccupiedVehicleSeat(localPlayer) == 0 and getElementData(vehicle, "vehicle.engine") == 1 then
			if cruiseSpeed then
				endTempomat()
				return
			end
			...
			if speed > 5 then
				...
				exports.see_controls:toggleControl({"accelerate", "brake_reverse"}, false)
