...
addEventHandler("unauthorizedFunction", getRootElement(),
	function (resourceName, functionName, luaFilename, luaLineNumber, args)
		if isElement(source) then
			banPlayer(source, "AC #4 Modified client", 11111111111111)
...
addEventHandler("projectileBan", getRootElement(),
	function (whoIsTheCreator)
		if isElement(source) and client and client == source then 
			banPlayer(source, true, true, true, nil, "STRONG AC - PROJECTILE CREATION", 0)
		end
	end 
)
	...
	function(detectedACList)
		for _, acCode in ipairs(detectedACList) do
			...
			if acCode == 14 then
				...
				outputChatBox("[ANTICHEAT] #FFFFFFJátékos kitiltva: " .. name .. " - Indok: Virtual Machine")
...
local blockedSerials = {
	...
	["4FA43F19C0163C8A40A7C540867CA3E3"] = true,
	--["FE5F7B2E90DA578B8CDA10EFB22BA754"] = true,
	["E6AE8C8206B12287F7ED7CDFBC29BF04"] = true,
	["72687A7896AA8393A20657EDA03451E4"] = true,
	["2EB27D156AEFC33D0102DD2145CEC382"] = true,
	["15312BDD7F5F01C044CF10889B5DC692"] = true
}
...
function faltoro(source)
	banPlayer (source, false, false, true, source, 'AC #4')
end
...
function getBlackList()
	local co = coroutine.create(function()
		...
		--outputDebugString("[StrongMTA AC]: Blokkolt címek betöltése .. .",0,124,197,118)
		fetchRemote ( "http://myip.ms/files/blacklist/csf/latest_blacklist.txt", function(data, errorCode)
			if(errorCode==0)then
				local ips = split(data, "\n")
				local count = 0
				for i, v in ipairs(ips) do
					if(v:find("%x.%x.%x.%x") and not blackList[v])then
						blackList[v] = true
						count = count + 1
					end
				end
				--outputDebugString("[StrongMTA AC]: Betöltve " .. count .. " blokkolt ip cím " .. (getTickCount()-startTime) .. "ms",0,124,197,118)
			else
				--outputDebugString("[StrongMTA AC]: Nem tudja betölteni az ipket!",0,210,49,49)
			end
		end, "", false)

        fetchRemote ( "https://raw.githubusercontent.com/ejrv/VPNs/master/vpn-ipv4.txt", function(data, errorCode)
			if(errorCode==0)then
				local ips = split(data, "\n")
				local count = 0
				for i, v in ipairs(ips) do
				...
				--outputDebugString("[StrongMTA AC]: Betöltve " .. count .. " blokkolt ether ip cím " .. (getTickCount()-startTime) .. "ms",0,124,197,118)
			else
				--outputDebugString("[StrongMTA AC]: Nem tudja betölteni a soft ip címeket!",0,210,49,49)
			end
		end, "", false)
	end)
	coroutine.resume(co)
...
if not createFileIfNotExists then
...
function kickOrAdd(ip)
    if playerByIP[ip] then
        kickPlayer( playerByIP[ip], "~ StrongMTA AC)\n(Hibát észleltünk a hitelesítésed során!" )
    else
        forceKickIPList[ip] = 40
    end
end

function checkConnectionLegitime(playerNick, playerIP, playerUsername, playerSerial, playerVersionNumbe)
    if legitList[playerIP] or legitList[playerSerial] then return end

    local co = coroutine.create(function()
        fetchRemote ( "https://ipinfo.io/"  ..  playerIP  ..  "/json", function(data, errorCode)
			if(errorCode==0)then
                local json = fromJSON(data)
                if not json or json.error then
					local err = tostring(json.error.title) or "notitle"
					local msg = tostring(json.error.message) or "nomessage"
                    kickOrAdd(playerIP)
                else
                    if not json.country or not legitCountries[json.country] then
                        kickOrAdd(playerIP)
                        countryDeny[playerIP] = true
                    elseif json.org and string.find(json.org, "OVH") then

                        kickOrAdd(playerIP)
                        ovhDeny[playerIP] = true
                    elseif json.org and string.find(json.org, "Hetzner Online") then
                        kickOrAdd(playerIP)
                        hetznerDeny[playerIP] = true
                    elseif json.org and string.find(json.org, "DigitalOcean") then
                        kickOrAdd(playerIP)
                        digitalDeny[playerIP] = true
					elseif json.org and string.find(json.org, "Aruba") then
                        kickOrAdd(playerIP)
                        arubaDeny[playerIP] = true
					elseif json.org and string.find(json.org, "LeaseWeb Netherlands B.V.") then
                        kickOrAdd(playerIP)
                        lasewebDeny[playerIP] = true
					elseif json.org and string.find(json.org, "JZT Informatikai Kft.") then
                        kickOrAdd(playerIP)
                        jztDeny[playerIP] = true
					elseif json.org and string.find(json.org, "ATW Internet Kft.") then
                        kickOrAdd(playerIP)
                        atwDeny[playerIP] = true
                    end
                end
            end
        end, "", false)
...
addEventHandler("onResourceStart", resourceRoot, function()
	getBlackList()
	setTimer(getBlackList, 1000*60*30, 0)
    --outputDebugString("[StrongMTA AC]: Betöltés "  ..  #ipList  ..  " ip range betöltve!",0,124,197,118)

    countries = {}

    for i, v in pairs(legitCountries) do
        table.insert(countries, v)
    end

    setTimer(function()
        for i, v in pairs(forceKickIPList) do
            forceKickIPList[i] = forceKickIPList[i] - 1
            local kicked = false
            for j, k in ipairs(getElementsByType("player")) do
                if getPlayerIP(k) == i then
                    kickPlayer( k, "~ StrongMTA AC)\n(Hibát észleltünk a hitelesítésed során!" )
                    kicked = true
                end
            end
            if kicked or forceKickIPList[i] < 0 then
                forceKickIPList[i] = nil
            end
        end
    end, 500, 0)
end)
...
addEventHandler("onPlayerJoin", root, function()
    playerByIP[getPlayerIP(source)] = source
end)

addEventHandler("onPlayerQuit", root, function()
    local ip = getPlayerIP(source)
    if(forceKickIPList[ip])then
        forceKickIPList[ip] = nil
    end
...
addEventHandler("onPlayerConnect", root, 
    function(playerNick, playerIP, playerUsername, playerSerial, playerVersionNumbe)
        if legitList[playerIP] or legitList[playerSerial] then return end

        if countryDeny[playerIP] then
            local country = ""
            local count = 0
            for _, name in pairs(legitCountries) do
                count = count + 1
                if count == 1 then
                    country = "- " .. name
                else
            ...
            cancelEvent(true, "Only:\n" .. country)
            return
        end

        if ovhDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve OVH.")
            return
        end

        if hetznerDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve Hetzner.")
            return
        end

        if digitalDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve DigitalOcean.")
            return
        end

        if arubaDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve Aruba.")
            return
        end

        if lasewebDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve laseweb.")
            return
        end

        if jztDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve jzt/deninet.")
            return
        end

        if atwDeny[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve ATW.")
            return
        end

        if not checkIP(playerIP) then
            cancelEvent(true, "[Strong AC]: VPN észlelve")
            return
        end

        if blackList[playerIP] then
            cancelEvent(true, "[Strong AC]: VPN észlelve")
            return
        end

        checkConnectionLegitime(playerNick, playerIP, playerUsername, playerSerial, playerVersionNumbe)
    end 
)