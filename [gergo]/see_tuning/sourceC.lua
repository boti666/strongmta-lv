				...
				if selectedMenu == 0 then
					...
					if selectedSubMenu == 0 then
						...
						if currentMenu.id == "platina" then
							local textureCount = exports.see_platina:getPaintJobCount(getElementModel(vehicleElement))
			...
			if key == "backspace" then
				...
				if selectedMenu > 0 then
					...
					if currentMenu.id == "color" or currentMenu.colorPicker then
						setElementData(vehicleElement, "vehicleColor", {vehicleColor[1], vehicleColor[2], vehicleColor[3]})
						print("asd")
...
addEventHandler("buyPlatina", getRootElement(),
	function (state, value)
		if state == "success" then
			...
			exports.see_platina:applyPaintJob(value, true)
...
function startBuyProgress(typ, text)
	...
	if typ == "newpart" then
		buyProgressDuration = 6000 / 3
	elseif typ == "paint" then
		buyProgressDuration = 8050 / 3
	end
...
function installPreviewTuning()
	...
	if currentMenu.id == "platina" then
		...
		if currentSelection.value then
			exports.see_platina:applyPaintJob(currentSelection.value, false)
		end
	end
