			...
			if movedItem and type(movedItem) == "table" and data and type(data) == "table" then
				...
				if characterId and itemsTable[source] then
					...
					if slot then
						...
						outputChatBox("#DC143C[StrongMTA]: #ffffffA tárgy eldobása meghiúsult.", source, 0, 0, 0, true)
					end
				end
			end
		end
				...
				if worldItem then
					...
					if characterId then
						...
						if getCurrentWeight(source) + getItemWeight(worldItem.itemId) * worldItem.amount > getWeightLimit("player", source) then
							outputChatBox("#DC143C[StrongMTA]: #ffffffAz inventoryd nem bírja el a kiválasztott tárgyat!", source, 0, 0, 0, true)
						else
							local slotId = findEmptySlot(source, "player", worldItem.itemId)

							if not slotId then
								outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFNincs szabad slot az inventorydban!", source, 255, 255, 255, true)
							else
