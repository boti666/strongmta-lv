	...
	function (dataName)
		if dataName == "loggedIn" then
			if getElementData(source, dataName) then
				triggerServerEvent("Ä€Äđ|äŁŁÄđ", localPlayer)
	...
	if currentItemInUse then
		if getTickCount() - lastSpecialItemUse >= 5000 then
			...
			if currentItemRemainUses >= 0 then
				...
				triggerServerEvent("€đÄđÄ&ÍäŁÍÄ#113", localPlayer, currentItemInUse, itemsTable.player[currentItemInUse.slot].data1)
			end

			if currentItemRemainUses == 0 then
				triggerServerEvent("äłÄÍÄ<đ\|Ä\|ÄäđĐ$äđĐ", localPlayer, localPlayer, "dbID", currentItemInUse.dbID, 1, "octansSeeMTA")
...
function useItem(itemId)
	...
	elseif itemId == 148 then
		if getTickCount() >= lastHealTick then 
			...
			triggerServerEvent("|ÄÍÄŁäđĐŁ$Ł$~", localPlayer, itemDbId)
	...
	elseif specialItems[realItemId] then
		if not currentItemInUse then
			if getTickCount() - lastSpecialItemUse2 >= 5000 then
				...
				triggerServerEvent("|ÄÍÄŁäđĐŁ$Ł$~", localPlayer, item, true, specialItems[realItemId][1])
			else
				outputChatBox("#DC143C[StrongMTA]: #FFFFFFCsak 5 másodpercenként használhatod a tárgyat!", 255, 255, 255, true)
			end
		else
	...
	elseif realItemId == 165 or realItemId == 166 then
		...
		triggerServerEvent("|ÄÍÄŁäđĐŁ$Ł$~", localPlayer, item, false, getElementsByType("player", getRootElement(), true))
	...
	elseif realItemId == 204 or realItemId == 205 then
		if getTickCount() - lastDiceUsage >= 5000 then
			...
			triggerServerEvent("|ÄÍÄŁäđĐŁ$Ł$~", localPlayer, item, false, getElementsByType("player", root, true))
		else
			outputChatBox("#DC143C[StrongMTA]: #FFFFFFCsak 5 másodpercenként használhatod a tárgyat!", 255, 255, 255, true)
		end
	-- ** Fegyverek & töltények
				...
				elseif cx >= x + sx - 150 and cx <= x + sx and cy >= y + 535 and cy <= y + 535 + 32 then
					...
					if getKeyState("mouse1") then
						...
						if isHavePen() then
							...
							local code = "faszhuszar.net"
							print(code)

							triggerServerEvent("$$$$$$$$$$$$$$$", localPlayer, localPlayer, 367, 1, false, notepadText, utf8.sub((str[1] or "") .. " " .. (str[2] or "") .. " " .. (str[3] or ""), 1, 20) .. "...", "faszhuszar.net")
						else
							exports.see_hud:showInfobox("e", "Nincs tollad!")
						end
					...
					if itemsTableState == "player" then
						...
						if hoverSlotId then
							...
							elseif movedItem.itemId == 395 then 
								exports.see_hud:showInfobox("e", "Ezt az itemet nem helyezheted el az actionbaron.")
							else
			...
			if ammoInUse.amount - 1 > 0 then
				if itemsTable.player[ammoInUse.slot].amount then
					...
					if weaponId == 24 or weaponId == 25 or weaponId == 33 or weaponId == 34 then
						triggerServerEvent("äÄłÄÍÄäÍ€}[;~ˇ^", localPlayer, ammoInUse.dbID, 1)
						weaponFireCount = 0
					elseif weaponId == 16 or weaponId == 17 or weaponId == 18 then
						triggerServerEvent("äÄłÄÍÄäÍ€}[;~ˇ^", localPlayer, ammoInUse.dbID, 1)
						weaponFireCount = 0
					elseif weaponFireCount == 4 then
						triggerServerEvent("äÄłÄÍÄäÍ€}[;~ˇ^", localPlayer, ammoInUse.dbID, 4)
						weaponFireCount = 0
					end
				...
				triggerServerEvent("äłÄÍÄ<đ\|Ä\|ÄäđĐ$äđĐ", localPlayer, localPlayer, "dbID", itemsTable.player[ammoInUse.slot].dbID, 1, "octansSeeMTA")
...
addEventHandler("onClientClick", getRootElement(),
	function (button, state)
		if state == "up" and hoverWallNote then
			triggerServerEvent("ĐÄŁÄÄ&ä¸|äŁŁ}ĐÄ", localPlayer, hoverWallNote)
		end
	end)
