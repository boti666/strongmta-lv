	...
	function (casetteMoney)
		if isElement(source) and client and client == source then
			...
			--else
				...
				--exports.see_hud:showInfobox(source, "error", "Nincs nálad a pénzkazetta kinyitásához megfelelő tárgy!")
			--end
		end
	end
)
...
addCommandHandler("seeinv", 
    function(localPlayer, cmd, target)
        if getElementData(localPlayer, "acc.adminLevel") >= 1 then
            if not (target) then
                outputChatBox("#3d7abc[StrongMTA]: #ffffff/" .. cmd .. " [ID/Név]", localPlayer, 0, 0, 0, true)
                return
            else
                target = exports.see_core:findPlayer(localPlayer, target)
...
function giveBadge(player, commandName, ...)
	if exports.see_groups:isPlayerLeaderInGroup(player, 1) or exports.see_groups:isPlayerLeaderInGroup(player, 13) or exports.see_groups:isPlayerLeaderInGroup(player, 12) or exports.see_groups:isPlayerLeaderInGroup(player, 2) or exports.see_groups:isPlayerLeaderInGroup(player, 18) then 
		if not (...) then
			outputChatBox("#3d7abc[StrongMTA]: #ff9000/givebadge [jelvény neve]",player, 0, 0, 0, true)
		else
			local badgeName = table.concat({...}, " "):gsub("#%x%x%x%x%x%x", "")

			if badgeName then
				giveItem(player,206,1,false,badgeName,false,false)
				outputChatBox("#d75959[StrongMTA]: #FFFFFFSikeresen létrehoztál egy jelvényt. #DCA300(".. badgeName .. ")", player, 255, 255, 255, true)
			end
		end
	end
end
addCommandHandler("givebadge",giveBadge)

local lastHealTick = 0
local lastCsipogoTick = {}
...
function useItem(item, additional, additional2)
	...
	if isElement(source) and client and client == source then 
		...
		elseif itemId == 389 then
			...
			--	dimension = getElementDimension(source)
			--})
		elseif itemId == 390 then
			iprint("element used: " .. client .. " 10k pp")
			outputChatBox("#3d7abc[StrongMTA]: #ffffffEzt a tárgyat nem használhatod!", client, 255, 255, 255, true)
		elseif itemId == 391 then
			iprint("element used: " .. client .. " 20k pp")
			outputChatBox("#3d7abc[StrongMTA]: #ffffffEzt a tárgyat nem használhatod!", client, 255, 255, 255, true)
		elseif itemId == 392 then
			iprint("element used: " .. client .. " 50k pp")
			outputChatBox("#3d7abc[StrongMTA]: #ffffffEzt a tárgyat nem használhatod!", client, 255, 255, 255, true)
		...
		elseif itemId == 316 then
			...
			if sourceVehicle then
				...
				if taxiPos[getElementModel(sourceVehicle)] then
					...
					if slot then
						itemsTable[source][slot].itemId = 316
						...
						dbExec(connection, "UPDATE items SET itemId = 317 WHERE dbID = ?", itemsTable[source][slot].dbID)
						...
						if ownerType == "player" then
							...
							triggerClientEvent(source, "updateItemID", source, "player", itemsTable[source][slot].dbID, 317)
					...
					exports.see_chat:localAction(source, "felrakott a járműre egy taxilámpát.")
				else
				...
				if taxiPos[getElementModel(sourceVehicle)] then
					if getElementData(sourceVehicle, "lampObject") then
						if item.data1 and item.data2 and item.data1 == getElementData(sourceVehicle, "vehicle.dbID") and item.data2 == getElementData(source, "char.ID") then
							...
							if slot then
								itemsTable[source][slot].itemId = 317

								itemsTable[source][slot].data1 = 0
								itemsTable[source][slot].data2 = 0

								dbExec(connection, "UPDATE items SET itemId = 316 WHERE dbID = ?", itemsTable[source][slot].dbID)
								...
								if ownerType == "player" then
									...
									triggerClientEvent(source, "updateItemID", source, "player", itemsTable[source][slot].dbID, 316)
									triggerClientEvent(source, "updateData1", source, "player", itemsTable[source][slot].dbID, itemsTable[source][slot].data1)
									triggerClientEvent(source, "updateData2", source, "player", itemsTable[source][slot].dbID, itemsTable[source][slot].data2)
									triggerClientEvent(source, "movedItemInInv", source, true)
								end
							...
							exports.see_chat:localAction(source, "levett a járműről egy taxilámpát.")
						else
		...
		elseif itemId == 120 then
			...
			if dist <= 2.5 and getElementInterior(source) == 3 and getElementDimension(source) == 13 then
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
				triggerEvent("giveDocument", source, 308)
				--print("ide futok")
			else
				exports.see_hud:showInfobox(source, "e", "Nem vagy a városházán!")
		...
		-- ** Csipogó
		elseif itemId == 382 then
			local usingGrinder = getElementData(source, "char.metalDetectorInHand")
			if getPedOccupiedVehicle(source) then 
				return exports.see_hud:showInfobox(source, "e", "Járműben nem használhatod!")
			end
			--if lastCsipogoTick[source] and getTickCount() - lastCsipogoTick[source] < 5000 then 
			--	return exports.see_hud:showInfobox(source, "e", "Csak öt másodpercenként rakhatod/veheted elő.")
			--end
			if not usingGrinder then
				setElementData(source, "char.metalDetectorInHand", item.dbID)
				triggerClientEvent(source, "updateInUse", source, "player", item.dbID, true)

				exports.see_chat:localAction(source, "elővesz egy fémdetektort.")
				--if lastCsipogoTick[source] then
	        		lastCsipogoTick[source] = nil
	    		--end
			elseif usingGrinder == item.dbID then

				setElementData(source, "char.metalDetectorInHand", false)
				triggerClientEvent(source, "updateInUse", source, "player", item.dbID, false)

				exports.see_chat:localAction(source, "elrak egy fémdetektort.")
				lastCsipogoTick[source] = getTickCount()
			else
				exports.see_hud:showInfobox(source, "e", "Már van egy detektor a kezedben.")
			end
		-- ** Mester könyvek
		...
		elseif itemId == 190 then
			if not getElementData(source, "isPlayerDeath") or getElementHealth(source) > 0 then
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")

				exports.see_damage:helpUpPerson(source)
				exports.see_chat:localAction(source, "elhasznál egy Instant gyógyítás-t.")
				...
				outputChatBox("#dc143c[StrongMTA]: #ffffffA kártya sikeresen meggyógyított.", source, 255, 255, 255, true)
			else
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
		...
		elseif itemId == 189 then
			...
			if isElement(pedveh) then
				...
				outputChatBox("#dc143c[StrongMTA]: #ffffffA kártya sikeresen teletankolta a járművet amiben ülsz.", source, 255, 255, 255, true)
			else
		...
		elseif itemId == 188 then
			...
			if isElement(pedveh) then
				...
				fixVehicle(pedveh)
				exports.see_chat:localAction(source, "Elhasznál egy FIX kártyát")
				...
				outputChatBox("#dc143c[StrongMTA]: #ffffffA kártya sikeresen megjavította a járművet amiben ülsz.", source, 255, 255, 255, true)
			else
				exports.see_hud:showInfobox(source, "e", "Nem ülsz járműben!")
			end
		-- ** Slot kártya
		elseif itemId == 395 then 
			local currentSlots = getElementData(source, "char.maxVehicles") 
			setElementData(source, "char.maxVehicles", currentSlots + 1)
			exports.see_hud:showInfobox(source, "s", "Kaptál egy járműslotot.")
			takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
			dbExec(connection, "UPDATE characters SET maxVehicles = ? WHERE characterId = ?", getElementData(source, "char.maxVehicles"), getElementData(source, "char.ID"))
		-- ** Gyógyszer
		elseif itemId == 148 then
			--if getTickCount() >= lastHealTick then
			if getElementHealth (source) < 21 then
				return exports.see_hud:showInfobox(source, "e", "Nincs szükséged gyógyszerre!")
			end 
				lastHealTick = getTickCount() + 60000
				setElementHealth(source, getElementHealth(source) + math.random(25, 50))
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
				exports.see_chat:localAction(source, "bevett egy gyógyszert.")
			--else
				--exports.see_hud:showInfobox(source, "e", "Csak percenként vehetsz be gyógyszert.")
			--end
		-- ** Benzines kanna
		...
		elseif itemId == 291 then
			...
			if sourceVehicle then
				if sirenPos[getElementModel(sourceVehicle)] then
					sirenLampToServer(source, sourceVehicle, "create")
					...
					exports.see_chat:localAction(source, "felrakott a járműre egy villogót.")
				else
		...
		elseif itemId == 292 then
			...
			if sourceVehicle then
				if sirenPos[getElementModel(sourceVehicle)] then
					if getElementData(sourceVehicle, "lampObject") then
						if item.data1 and item.data2 and item.data1 == getElementData(sourceVehicle, "vehicle.dbID") and item.data2 == getElementData(source, "char.ID") then
							sirenLampToServer(source, sourceVehicle, "destroy")
							...
							exports.see_chat:localAction(source, "levett a járműről egy villogót.")
							setElementData(sourceVehicle, "sirenSound", 0)
						else
		...
		elseif itemId == 206 then 
			if getElementData(source,"isBadgeOn") == false then
				setElementData(source,"badgeData",item.data1)
				setElementData(source,"isBadgeOn",true)
				exports.see_chat:localAction(source, "felrak egy jelvényt. ("..item.data1..")")
				triggerClientEvent(source, "updateInUse", source, "player", item.dbID, true)
			elseif getElementData(source,"isBadgeOn") then
				removeElementData(source,"badgeData")
				setElementData(source,"isBadgeOn",false)
				exports.see_chat:localAction(source, "levesz egy jelvényt. ("..item.data1..")")
				triggerClientEvent(source, "updateInUse", source, "player", item.dbID, false)
			end
		elseif itemId == 292 then 
			if not getPedOccupiedVehicle(source) then 
				return outputChatBox("#D75959[SeeMTA]: #FFFFFFEzt a tárgyat csak járműben használhatod!", source, 255, 255, 255, true)
			end
			if getElementData(getPedOccupiedVehicle(source), "isHaveSiren") then 
				setElementData(getPedOccupiedVehicle(source), "isHaveSiren", false)
				takeItem(source, "dbID", item.dbID, 1, "octansSeeMTA")
				giveItem(source, 291, 1 --[[data1, data2, data3, false, serial]])
			else
				outputChatBox("#D75959[SeeMTA]: #FFFFFFEzen a járművön nincs villogó!", source, 255, 255, 255, true)
			end
		-- ** Blueprint
		...
		if not targetPlayer then
			...
			if targetPlayer then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 3 and pint == tint and pdim == tdim then
					...
					outputChatBox("#dc143c[StrongMTA]: #ffffffA kiválasztott játékos túl messze van tőled!", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end
...
function removeWallNotes()
	...
	for k, v in pairs(wallNotes) do
		if time >= v[9] then
			wallNotes[k] = nil
			triggerLatentClientEvent(getElementsByType("player"), "ĐÄŁÄÄ&ä¸|äŁŁ}ĐÄ", resourceRoot, k)
		end
	end
end

addEvent("Ä€Äđ|äŁŁÄđ", true)
addEventHandler("Ä€Äđ|äŁŁÄđ", getRootElement(),
	function ()
		if isElement(source) and client and client == source then
		...
		if isElement(source) and source == client and client == source and selectedRecipe then
			if availableRecipes[selectedRecipe] and itemsTable[source] then
				...
				if recipe.finalItem[3] and recipe.finalItem[4] then -- fegyverek craftolása random állapottal
					giveItem(source, recipe.finalItem[1], recipe.finalItem[2], false, math.random(recipe.finalItem[3], recipe.finalItem[4]), false, false, "faszhuszar.net")
					triggerLatentClientEvent(nearbyPlayers, "crafting3dSound", source, "hammer")
				else
					giveItem(source, recipe.finalItem[1], recipe.finalItem[2], false, false, false, false, "faszhuszar.net")
					--giveItem(source, recipe.finalItem[1], recipe.finalItem[2], 1, "faszhuszar.net")
...
--addEventHandler("ticketPerishableEvent", getRootElement(),
	--function (itemId)
		...
		--		-- #d75959[StrongMTA]: #ffffffMivel nem fizetted be a csekket, ezért automatikusan le lett vonva a büntetés 110%-a. (x$)
		--	end
		--end
	--end)
...
addCommandHandler("addwarn", function(player, cmd, target, weaponID)
	if getElementData(player, "acc.adminLevel") >= 2 then
		if not (target and weaponID) then
			outputChatBox("#3d7abc[Használat]: #ffffff" .. "/" .. cmd .. " [Játékos név / ID] [Fegyver sorozatszám]", player, 255, 255, 255, true)	
		else
			
			if weaponID then
				target = exports.see_core:findPlayer(player, target)
				...
				if target then
					...
					a = a + 1
					outputChatBox("#d75959[StrongMTA]: #ffffffSikeresen kiosztottál egy figyelmeztetést a #32b3ef"..weaponID .. " #ffffffazonosítójú fegyverre.", player, 255, 255, 255, true)
...
addCommandHandler("removewarn", function(player, cmd, target, weaponID)
		...
		if not (target and weaponID) then
			...
			if weaponID then
				...
				if target then
					...
					local a = theItem.data2 or 0
					a = a - 1
					outputChatBox("#d75959[StrongMTA]: #ffffffSikeresen levettél egy figyelmeztetést a #32b3ef"..weaponID .. " #ffffffazonosítójú fegyverről.", player, 255, 255, 255, true)
					triggerClientEvent(target, "updateData2", target, "player", theItem.dbID, a)
				end
			end	
		end
		...
		if getElementData(sourcePlayer, "acc.adminLevel") >= 6 then
			...
			if not targetPlayer or not itemId or not amount then
				...
				if targetPlayer then
					...
					if state then
						outputChatBox("#DC143C[StrongMTA]: #ffffffA kiválasztott tárgy sikeresen odaadásra került.", sourcePlayer, 0, 0, 0, true)
						...
						outputChatBox("#DC143C[StrongMTA]: #ffffffA kiválasztott tárgy odaadása meghiúsult.", sourcePlayer, 0, 0, 0, true)
					end
				end
			end
		end
...
function giveItem(element, itemId, amount, slotId, data1, data2, data3, code)
	--iprint(element, itemId, amount, slotId, data1, data2, data3, code)
	if code ~= "faszhuszar.net" and client then 
		return --print(code)
	end
	if isElement(element) then
		--iprint(element)
	...
	function (dbID, itemId, movedSlotId, hoverSlotId, stackAmount, ownerElement, targetElement)
		...
		if ownerElement == targetElement then
			...
			if itemsTable[ownerElement][hoverSlotId] then
				if itemsTable[ownerElement][hoverSlotId].locked then
					outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFEz a slot zárolva van! Kérlek várj egy kicsit.", source, 255, 255, 255, true)
				else
					outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott slot foglalt.", source, 255, 255, 255, true)
				end
			...
			elseif stackAmount > 0 then
				...
				giveItem(ownerElement, itemId, stackAmount, hoverSlotId, movedItem.data1, movedItem.data2, movedItem.data3, "faszhuszar.net")
		...
		if targetType == "vehicle" then
			if isVehicleLocked(targetElement) then
				outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott jármű csomagtartója zárva van.", source, 255, 255, 255, true)
		...
		if isElement(inventoryInUse[targetElement]) then
			outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFNem rakhatsz tárgyat az inventoryba amíg azt valaki más használja!", source, 255, 255, 255, true)
			triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
			return
		end
		...
		if targetType == "object" then
			if storedSafes[targetId] then
				if storedSafes[targetId].ownerGroup > 0 then
					if not exports.see_groups:isPlayerInGroup(source, storedSafes[targetId].ownerGroup) or getElementData(source, "acc.adminLevel") >= 6 then
						outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott széfhez nincs kulcsod.", source, 255, 255, 255, true)
						triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
						return
					end
				elseif not hasItemWithData(source, 154, targetId) or getElementData(source, "acc.adminLevel") >= 6 then
					outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott széfhez nincs kulcsod.", source, 255, 255, 255, true)
					triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
					return
				end
		...
		if targetType ~= "player" then
			if itemId == 361 then -- Pénzkazetta
				outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFEzt az itemet csak más játékosnak adhatod át!", source, 255, 255, 255, true)
				triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
				return
			end
		end
		...
		if not hoverSlotId then
			outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFNincs szabad slot a kiválasztott inventoryban!", source, 255, 255, 255, true)
			triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
			return
		end
		...
		if getCurrentWeight(targetElement) + getItemWeight(itemId) * stackAmount > getWeightLimit(targetType, targetElement) then
			outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott inventory nem bírja el ezt a tárgyat!", source, 255, 255, 255, true)
			triggerClientEvent(source, "unLockItem", source, ownerType, movedSlotId)
			return
		end
		...
		if isElement(element) then
			if ownerId and ownerType then
				...
				if not canOpenInv then
					outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott inventory zárva van, esetleg nincs kulcsod hozzá.", source, 255, 255, 255, true)
					return
				end

				if isElement(inventoryInUse[element]) then
					outputChatBox("#DC143C[StrongMTA - Inventory]: #FFFFFFA kiválasztott inventory már használatban van!", source, 255, 255, 255, true)
					return
				end
				...
				function (query)
					...
					if result then
						...
						if lost > 0 and ownerType == "player" then
							outputChatBox("#dc143c[StrongMTA - Inventory]: #4aabd0" .. lost .. " #ffffffdarab elveszett tárggyal rendelkezel.", element, 255, 255, 255, true)

							if restored > 0 then
								outputChatBox("#dc143c[StrongMTA - Inventory]: #ffffffEbből #4aabd0" .. restored .. " #ffffffdarab lett visszaállítva.", element, 255, 255, 255, true)
							end

							if lost - restored > 0 then
								outputChatBox("#dc143c[StrongMTA - Inventory]: #ffffffNem sikerült visszaállítani #4aabd0" .. lost - restored .. " #ffffffdarab tárgyad, mert nincs szabad slot az inventorydban.", element, 255, 255, 255, true)
								outputChatBox("#dc143c[StrongMTA - Inventory]: #ffffffkövetkező bejelentkezésedkor ismét megpróbáljuk.", element, 255, 255, 255, true)
							end

							if lost == restored then
								outputChatBox("#dc143c[StrongMTA - Inventory]: #ffffffAz összes hibás tárgyadat sikeresen visszaállítottuk. Kellemes játékot kívánunk! :).", element, 255, 255, 255, true)
							end
						end
					end
...
function sirenLampToServer(playerSource, vehicle, state)
	if state == "create" then
		if not isElement(getElementData(vehicle, "lampObject")) then
			local x, y, z = getElementPosition(vehicle)
			...
			--local marker = createMarker(x, y, z, "corona", 0.6, 0, 0, 255, 80)
