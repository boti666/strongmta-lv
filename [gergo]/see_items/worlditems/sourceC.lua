...
addCommandHandler("nearbyitems",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben lévő tárgyak:", 255, 255, 255, true)
			...
			for k, v in pairs(worldItems) do
				if int == v.interior and dim == v.dimension then
					...
					if dist <= 15 then
						outputChatBox("#DC143C[StrongMTA]: #FFFFFFAzonosító: #8dc63f" .. v.dbID .. "#FFFFFF <> Távolság: #8dc63f" .. math.floor(dist) .. "#FFFFFF <> Típus: #8dc63f" .. getItemName(v.itemId), 255, 255, 255, true)
						nearby = nearby + 1
					end
				end
			end
			...
			if nearby == 0 then
				outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben nem található egyetlen tárgy sem.", 255, 255, 255, true)
			end
		end
	end)