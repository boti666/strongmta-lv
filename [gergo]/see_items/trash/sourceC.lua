...
addCommandHandler("nearbytrashes",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben lévő szemetesek:", 255, 255, 255, true)
			...
			for k, v in pairs(storedTrashes) do
				if int == v.interior and dim == v.dimension then
					...
					if dist <= 15 then
						outputChatBox("#DC143C[StrongMTA]: #FFFFFFAzonosító: #8dc63f" .. v.trashId .. "#FFFFFF <> Távolság: #8dc63f" .. math.floor(dist), 255, 255, 255, true)
						nearby = nearby + 1
					end
				end
			end
			...
			if nearby == 0 then
				outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben nem található egyetlen szemetes sem.", 255, 255, 255, true)
			end
		end
	end)