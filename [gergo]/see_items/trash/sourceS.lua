				...
				dbQuery(
					function (qh)
						...
						if result then
							...
							outputChatBox("#DC143C[StrongMTA]: #ffffffA szemetes sikeresen létrehozva. #4aabd0(" .. result.trashId .. ")", sourcePlayer, 0, 0, 0, true)
						else
							outputChatBox("#DC143C[StrongMTA]: #ffffffA szemetes létrehozása meghiúsult.", sourcePlayer, 0, 0, 0, true)
						end
					end,
				connection, "SELECT * FROM trashes WHERE trashId = LAST_INSERT_ID()")
			else
				outputChatBox("#DC143C[StrongMTA]: #ffffffA kiválasztott model nem egy szemetes model.", sourcePlayer, 0, 0, 0, true)
			end
		end
	end)
	...
	function (sourcePlayer, commandName, trashId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 7 then
			...
			elseif not storedTrashes[trashId] then
				outputChatBox("#DC143C[StrongMTA]: #ffffffA kiválasztott szemetes nem létezik.", sourcePlayer, 0, 0, 0, true)
			else
				...
				outputChatBox("#3d7abc[StrongMTA]: #ffffffA szemetes sikeresen törölve.", sourcePlayer, 0, 0, 0, true)
			end
		end
	end)
