		...
		if getElementData(sourcePlayer, "acc.adminLevel") >= 7 then
			...
			if not safeId then
				...
				if data then
					...
					outputChatBox("#DC143C[StrongMTA]: #FFFFFFA kiválasztott széf nem található.", sourcePlayer, 255, 255, 255, true)
				end
			end
		end
	end)
	...
	function (safeId, sourcePlayer)
		if isElement(source) then
			...
			if safeId then
				...
				outputChatBox("#3d7abc[StrongMTA]: #ffffffA széf sikeresen törölve.", source, 0, 0, 0, true)
			end
		end
	end)
