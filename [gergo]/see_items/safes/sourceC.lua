		...
		if getElementData(localPlayer, "acc.adminLevel") >= 7 then
			...
			if not safeId then
				...
				if storedSafes[safeId] then
					...
					outputChatBox("#DC143C[StrongMTA]: #FFFFFFA kiválasztott széf nem található.", 255, 255, 255, true)
				end
			end
		end
	end)
...
addCommandHandler("nearbysafes",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben lévő széfek:", 255, 255, 255, true)
			...
			for k, v in pairs(storedSafes) do
				if int == v.interior and dim == v.dimension then
					...
					if dist <= 15 then
						outputChatBox("#DC143C[StrongMTA]: #FFFFFFAzonosító: #8dc63f" .. v.dbID .. "#FFFFFF <> Csoport Azonosító: #8dc63f" .. v.ownerGroup .. "#FFFFFF <> Távolság: #8dc63f" .. math.floor(dist), 255, 255, 255, true)
						nearby = nearby + 1
					end
				end
			end
			...
			if nearby == 0 then
				outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben nem található egyetlen széf sem.", 255, 255, 255, true)
			end
		end
	end)