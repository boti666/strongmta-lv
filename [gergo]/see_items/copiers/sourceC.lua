...
addCommandHandler("nearbycopiers",
	function ()
		if getElementData(localPlayer, "acc.adminLevel") >= 1 then
			...
			outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben lévő fénymásolók:", 255, 255, 255, true)
			...
			for k, v in pairs(storedCopiers) do
				if int == v.interior and dim == v.dimension then
					...
					if dist <= 15 then
						outputChatBox("#DC143C[StrongMTA]: #FFFFFFAzonosító: #8dc63f" .. v.copierId .. "#FFFFFF <> Távolság: #8dc63f" .. math.floor(dist), 255, 255, 255, true)
						nearby = nearby + 1
					end
				end
			end
			...
			if nearby == 0 then
				outputChatBox("#DC143C[StrongMTA]: #FFFFFFA közeledben nem található egyetlen fénymásoló sem.", 255, 255, 255, true)
			end
		end
	end)
