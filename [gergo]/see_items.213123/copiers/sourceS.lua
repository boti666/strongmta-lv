			...
			dbQuery(
				function (qh, sourcePlayer)
					...
					if result then
						...
						outputChatBox("#DC143C[SeeMTA]: #ffffffA fénymásoló sikeresen létrehozva. #4aabd0(" .. result.copierId .. ")", sourcePlayer, 0, 0, 0, true)
					else
						outputChatBox("#DC143C[SeeMTA]: #ffffffA fénymásoló létrehozása meghiúsult.", sourcePlayer, 0, 0, 0, true)
					end
				end, {source},
	...
	function (sourcePlayer, commandName, copierId)
		if getElementData(sourcePlayer, "acc.adminLevel") >= 7 then
			...
			elseif not storedCopiers[copierId] then
				outputChatBox("#DC143C[SeeMTA]: #ffffffA kiválasztott fénymásoló nem létezik.", sourcePlayer, 0, 0, 0, true)
			else
				...
				outputChatBox("#7cc576[SeeMTA]: #ffffffA fénymásoló sikeresen törölve.", sourcePlayer, 0, 0, 0, true)
			end
		end
	end)
