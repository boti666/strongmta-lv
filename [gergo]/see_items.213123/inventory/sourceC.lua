...
function craftDone(item)
	if item[1] and item[2] then
		outputChatBox("#7cc576[SeeMTA]: #FFFFFFSikeresen elkészítetted a kiválasztott receptet! #32b3ef(" .. getItemName(item[1]) .. ")", 255, 255, 255, true)
...
function useSpecialItem()
	if currentItemInUse then
		if getTickCount() - lastSpecialItemUse >= 5000 then
			...
			outputChatBox("#DC143C[SeeMTA]: #FFFFFFCsak 5 másodpercenként használhatod a tárgyat!", 255, 255, 255, true)
		end
	end
end
...
function useItem(itemId)
	...
	elseif realItemId == 115 then
		if exports.see_groups:isPlayerHavePermission(localPlayer, "doorRammer") then
			...
			outputChatBox("#d75959[SeeMTA]: #ffffffNem használhatod a faltörő kost.", 255, 255, 255, true)
		end
	-- ** Hi-Fi
	elseif realItemId == 150 then
		...
		if not nearbyHifi then
			...
			outputChatBox("#d75959[SeeMTA]: #ffffffMár van egy rádió a közeledben!", 255, 255, 255, true)
		end
	-- ** Golyó bevizsgálás
	...
	elseif isWeaponItem(realItemId) or isAmmoItem(realItemId) then
		...
		if isWeaponItem(realItemId) then
			...
			if getPedControlState("fire") then
				outputChatBox("#7cc576[SeeMTA]: #ffffffAmíg nyomva tartod a lövés gombot, nem veheted elő a fegyvert.", 255, 255, 255, true)
				return
			end
	...
	function ()
		...
		if notepadState or noteState then
			...
			if notepadState then
				...
				dxDrawRectangle(x + sx - 150 + 4, y + 535 + 4, 142, 24, tocolor(124, 197, 118, 160))
			...
			if licenseData.iscopy then
				...
				if dist <= 2.5 and getElementInterior(localPlayer) == 3 and getElementDimension(localPlayer) == 13 then
					...
					if hoverRenewLicense then
						dxDrawRectangle(x + 4, y + 4, 142, 24, tocolor(124, 197, 118, 255))
					else
						dxDrawRectangle(x + 4, y + 4, 142, 24, tocolor(124, 197, 118, 175))
					end
	...
	function (button, state, absX, absY, worldX, worldY, worldZ, hitElement)
		...
		if button == "left" then
			...
			if state == "down" then
				...
				if currentTab == "crafting" and hoverCraftButton and not craftingProcess then
					...
					if not playerRecipes[selectedRecipe] and not defaultRecipes[selectedRecipe] then
						if not availableRecipes[selectedRecipe].requiredJob then
							outputChatBox("#DC143C[SeeMTA]: #FFFFFFEzt a receptet még nem tanultad meg!", 255, 255, 255, true)
							return
						end
					end
				...
				if hoverSlotId and itemsTable[itemsTableState][hoverSlotId] then
					...
					if not itemsTable[itemsTableState][hoverSlotId].inUse then
						...
						outputChatBox("#DC143C[SeeMTA]: #FFFFFFHasználatban lévő itemet nem mozgathatsz!", 255, 255, 255, true)
					end
				end
					...
					if not isPointOnActionBar(absX, absY) and not isPointOnInventory(absX, absY) then
						...
						if not isElement(hitElement) then
							...
							if dist <= 5 then
								...
								elseif isItemDroppable(movedItem.itemId) and not movedItem.dropped then
									...
									outputChatBox("#DC143C[SeeMTA]: #FFFFFFEzt a tárgyat nem lehet eldobni!", 255, 255, 255, true)
									playSound("files/sounds/select.mp3")
								end
							else
								outputChatBox("#DC143C[SeeMTA]: #FFFFFFIlyen messze nem dobhatsz el tárgyat!", 255, 255, 255, true)
								playSound("files/sounds/select.mp3")
							end
						...
						elseif elementType == "object" and isTrashModel(elementModel) then
							...
							if movedItem.data3 == "duty" then
								outputChatBox("#DC143C[SeeMTA]: #FFFFFFSzolgálati eszközzel ezt nem teheted meg!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.mp3")
								return
							end
							...
							if elementType == "player" and hitElement == localPlayer and itemsTableState == "player" then
								outputChatBox("#DC143C[SeeMTA]: #FFFFFFSaját inventoryból magadra nem húzhatsz itemet!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.mp3")
								return
							end
							...
							if (itemsTableState == "vehicle" or itemsTableState == "object") and (elementType ~= "player" or hitElement ~= localPlayer) then
								outputChatBox("#DC143C[SeeMTA]: #FFFFFFJárműből / széfből csak a saját inventorydba pakolhatsz!", 255, 255, 255, true)
								movedSlotId, haveMoving = false, false
								playSound("files/sounds/select.mp3")
								return
							end
							...
							if tonumber(elementId) then
								...
								outputChatBox("#DC143C[SeeMTA]: #FFFFFFA kiválasztott elem nem rendelkezik önálló tárterülettel!", 255, 255, 255, true)
								playSound("files/sounds/select.mp3")
							end
						end
					end
			...
			if itemsTableState == "player" and isKeyItem(movedItem.itemId) and hoverSlotId < defaultSettings.slotLimit then
				...
				outputChatBox("#DC143C[SeeMTA]: #FFFFFFEz az item átkerült a kulcsokhoz!", 255, 255, 255, true)
			end
			...
			if itemsTableState == "player" and isPaperItem(movedItem.itemId) and hoverSlotId < defaultSettings.slotLimit then
				...
				outputChatBox("#DC143C[SeeMTA]: #FFFFFFEz az item átkerült az iratokhoz!", 255, 255, 255, true)
			end
			...
			if hoverSlotId >= defaultSettings.slotLimit * 2 then
				if not isPaperItem(movedItem.itemId) then
					if isKeyItem(movedItem.itemId) then
						...
						outputChatBox("#DC143C[SeeMTA]: #FFFFFFEz nem irat!", 255, 255, 255, true)
					end
				end
			end
			...
			if hoverSlotId >= defaultSettings.slotLimit and hoverSlotId < defaultSettings.slotLimit * 2 then
				if not isKeyItem(movedItem.itemId) then
					if isPaperItem(movedItem.itemId) then
						...
						hoverSlotId = findEmptySlot("player")
						outputChatBox("#DC143C[SeeMTA]: #FFFFFFEz nem kulcs item!", 255, 255, 255, true)
					end
				end
			end
				...
				if not hoverSlotId and isElement(hitElement) and hitElement ~= localPlayer and hitElement ~= currentInventoryElement then
					...
					if elementType == "vehicle" then
						...
						if getPedOccupiedVehicle(localPlayer) then
							outputChatBox("#DC143C[SeeMTA]: #FFFFFFJárműben ülve nem nézhetsz bele a csomagtartóba!", 255, 255, 255, true)
							return
						end
...
function onRender()
	...
	dxDrawRectangle(panelPosX + 50, panelPosY + panelHeight - 20, (panelWidth - 50 - 10 - 60) * (weight / weightLimit), 10, tocolor(124, 197, 118))
	...
	dxDrawText(invTypText, panelPosX, panelPosY, panelPosX, panelPosY + 20, tocolor(124, 197, 118, 255), 0.55, Roboto, "left", "center")
	...
	if currentTab == "crafting" and itemsTableState ~= "vehicle" and itemsTableState ~= "object" then
		...
		for i = 0, 24 do
			...
			if x >= 1 and x <= 3 and y >= 1 and y <= 3 then
				if requiredRecipeItems and requiredRecipeItems[y] and requiredRecipeItems[y][x] then
					...
					if recipe[2] then
						dxDrawRectangle(x2, y2, defaultSettings.slotBoxWidth, defaultSettings.slotBoxHeight, tocolor(124, 197, 118, 200))
		...
		for i = 1, 8 do
			...
			if craft then
				...
				if craft[1] == "category" then
					...
					local color = craft[3] and tocolor(124, 197, 118) or textcolor
					...
					if craft[1] == "recipe" and craft[2] then
							...
							if not craftingProcess and selectedRecipe ~= id and cx >= x and cx <= x + sx and cy >= y2 and cy <= y2 + sy then
								...
								if recipe.requiredJob then
									if recipe.suitableColShapes then
										...
										showTooltip(cx, cy, "#ffffffMunka: #d75959Szükséges", "Pozíció: #7cc576Nem szükséges")
									end
								elseif recipe.requiredPermission then
									if recipe.suitableColShapes then
										showTooltip(cx, cy, "#ffffffCsoport: #d75959Szükséges", "Pozíció: #d75959Szükséges")
									else
										showTooltip(cx, cy, "#ffffffCsoport: #d75959Szükséges", "Pozíció: #7cc576Nem szükséges")
									end
						...
						if recipe then
							...
							elseif hoverRecipe == id or selectedRecipe == id then
								textcolor = tocolor(124, 197, 118, 175)
							end
		...
		if #craftList > 8 then
			...
			dxDrawRectangle(x + sx - 5, y + craftListOffset * (listHeight / (#craftList - 8 + 1)), 5, listHeight / (#craftList - 8 + 1), tocolor(124, 197, 118, 150))
		end
		...
		if selectedRecipe then
			...
			if craftStartTime then
				...
				if now >= craftStartTime then
					...
					dxDrawRectangle(x, y + listHeight + 4, sx * progress, sy - 4, tocolor(124, 197, 118))
				end
			end
	...
	-- ** Item slotok @ Itemek
		...
		for i = 0, defaultSettings.slotLimit - 1 do
			...
			if item and item.inUse then
				slotColor = tocolor(124, 197, 118, 200)
			end
		...
		if renameDetails then
			...
			dxDrawRectangle(renameDetails.x + 200 - 64, renameDetails.y + 3, 28, 18, tocolor(124, 197, 118, okayAlpha))
...
function bodySearchRender()
	...
	dxDrawText("Motozás:#ffffff " .. bodyName, searchPosX + 20, searchPosY, 0, searchPosY + 18, tocolor(124, 197, 118, 255), 0.5, Roboto, "left", "center", false, false, false, true)
	...
	elseif tonumber(currentMoney) > 0 then
		currentMoney = "#7cc576" .. math.abs(currentMoney)
	else
		currentMoney = 0
	end
...
function processTooltip(x, y, text, itemId, item, showItem)
	...
	elseif itemId == 309 then
		if tonumber(item.data3) == 0 then
			showTooltip(x, y, text, "Eredeti példány, rendszám: #7cc576" .. item.data2, showItem)
		else
			showTooltip(x, y, text, "Másolat, rendszám: #7cc576" .. item.data2, showItem)
		end
	elseif itemId == 412 then
		showTooltip(x, y, text, "Inmate " .. item.data1, showItem)
	elseif itemId == 206 then
		showTooltip(x, y, text, "#DCA300" .. item.data1 .. " # " .. item.data2, showTooltip)
...
function itemlistRender()
	...
	if #itemlistTable > 14 then
		...
		dxDrawRectangle(itemlistPosX + itemlistWidth - 5, itemlistPosY + 46 + (504 / #itemlistTable) * itemlistOffset, 5, (504 / #itemlistTable) * 14, tocolor(124, 197, 118, 150))
	end
